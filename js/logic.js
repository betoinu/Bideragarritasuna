...
window.updatePortfolio = function() {
    ...
    const textoLimpio = textoGastos.replace(/[^\d,.-]/g, '').replace(/\./g, '').replace(',', '.');
    const metaSupervivencia = safeNum(textoLimpio) || 0;
...
window.generatePDFReport = function() {
    ...
    setTimeout(() => {
        html2canvas(element, options).then(canvas => {
            ...
            while (heightLeft > 277) {
                pdf.addPage();
                currentPosition = 10 - (heightLeft - 277);
                pdf.addImage(canvas, 'PNG', 10, currentPosition, imgWidth, imgHeight);
                heightLeft -= 267;
            }
            ...
            console.log('✅ PDF-a ondo sortuta: ' + filename);
